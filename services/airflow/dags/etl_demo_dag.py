...
create_orders_dir = BashOperator(
    ...
    bash_command='mkdir -p /tmp/orders && sleep 30',
    dag=dag
)

create_customers_dir = BashOperator(
    task_id='create_customers_dir',
    bash_command='mkdir -p /tmp/customers && sleep 60',
    dag=dag
)

create_join_dir = BashOperator(
    task_id='create_join_dir',
    bash_command='mkdir -p /tmp/join_orders_and_customers && sleep 30',
    dag=dag
)
...
drop_orders_dir = BashOperator(
    ...
    bash_command='rm -rf /tmp/orders && sleep 30',
    dag=dag
)
...
create_orders_dir >> create_customers_dir >> create_join_dir
drop_orders_dir >> drop_customers_dir
